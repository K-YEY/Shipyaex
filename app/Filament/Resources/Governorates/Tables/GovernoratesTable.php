        ...
        return $table
            ->columns([
                ...
                TextColumn::make('created_at')
                    ...
                    ->visible(fn () => auth()->user()->isAdmin() || auth()->user()->can('ViewDatesColumn:Governorate'))
                    ->dateTime()
                    ->sortable()
            ...
            ->recordActions([
                EditAction::make()
                    ->visible(fn () => auth()->user()->isAdmin() || auth()->user()->can('Update:Governorate')),
            ])
            ->toolbarActions([
                BulkActionGroup::make([
                    DeleteBulkAction::make()
                        ->visible(fn () => auth()->user()->isAdmin() || auth()->user()->can('DeleteAny:Governorate')),
                ]),
            ]);
    }
}
